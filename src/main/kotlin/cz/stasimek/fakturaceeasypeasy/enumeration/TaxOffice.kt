...
enum class TaxOffice(
    private val regionalTaxOfficeId: Int,
    private val regionalTaxOfficeName: String,
    private val taxOfficeId: Int
) {
    ...
    SPECIALIZED_TO_CESKE_BUDEJOVICE(
        4022,
        "Specializovaný finanční úřad - územní pracoviště v Českých Budějovicích",
        13
    ),
            ...
            return Currency.values()
                ...
                .sortedWith(
                    Comparator<Pair<String, String>> { x, y ->
                        ...
                        if (x.first.startsWith("SPECIALIZED_TO_") && !y.first.startsWith("SPECIALIZED_TO_")) {
                            return@Comparator 1
                        }
                        if (!x.first.startsWith("SPECIALIZED_TO_") && y.first.startsWith("SPECIALIZED_TO_")) {
                            return@Comparator -1
                        }
                        val cityX = x.second.replaceFirst("Územní pracoviště (?:v|ve|pro)? ?(.+)".toRegex(), "$1")
                        val cityY = y.second.replaceFirst("Územní pracoviště (?:v|ve|pro)? ?(.+)".toRegex(), "$1")
                        val czech = Locale.of("cs", "CZ")
                        val collator = Collator.getInstance(czech) // Collator can sort Czech strings
                        collator.compare(cityX, cityY)
                    }
                )
                .toMap(LinkedHashMap())
        }
